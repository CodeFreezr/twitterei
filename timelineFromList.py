...
def get_all_tweets(screen_name): #Twitter only allows access to a users most recent 3240 tweets with this method
    ...
    new_tweets = api.list_timeline(owner_screen_name=listOwner,slug=listSlug)
    ...
    outtweets = [[tweet.id_str,tweet.user.screen_name,tweet.created_at, tweet.text.encode("utf-8"),tweet.retweet_count,tweet.favorite_count] for tweet in alltweets] #transform the tweepy tweets into a 2D array that will populate the csv    
    
    with open('%s_tweets.csv' % screen_name, 'wb') as f: #write the csv 
        writer = csv.writer(f, delimiter=';')
        writer.writerow(["id","author","created_at","text","retweet_count","favorite_count"])
